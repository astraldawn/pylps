...
def constraints_satisfied_new(o_goal, state, cycle_actions):
    # Create copy
    ...
    for constraint in KB.get_constraints(goal):
        ...
        for indiv_con in constraint:
            ...
            # Attempt to falsify
            c_satis = check_constraint_new(
                goal, indiv_con, combined_subs, sub_dict, all_actions)
...
def check_constraint_new(action, constraint, combined_subs,
                         sub_dict, current_actions):
    obj, state = constraint

    if obj.BaseClass is ACTION:
        '''
        TODO:
        This will not work, see action_defer_2
        '''
        if obj.name == action.name:
            vars_unified = 0
            vars_cnt = 0
        ...
        cycle_actions = []
        # KB.get_cycle_actions(r_obj)

        for c_action in current_actions:
            if c_action.name == r_obj.name:
                cycle_actions.append(c_action)

        # print('CYCLE ACTIONS', cycle_actions, r_obj)
        ...
        if var in sub_dict:
            ...
            for sub in sub_dict[var]:
                ...
                # action_exists = KB.exists_cycle_action(t_obj)
                action_exists = t_obj in current_actions
                t_sub_set.add(action_exists)
